...
Get `BerwinCode-Setup-1.6.7.exe` from the Releases page (recommended, no admin
rights), or the portable `BerwinCode-v1.6.7-windows-x64.zip`.
...
`BerwinCode-Setup-1.6.7.exe /KEEPOPENCODE=1`
...
```
powershell -ExecutionPolicy Bypass -File .\release.ps1 -Version 1.6.7
```
